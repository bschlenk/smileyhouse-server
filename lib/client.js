...
class Client extends EventEmitter {
    ...
    initialize() {
        ...
        this._socket.on('data', function dataHandler(data) {
            ...
            if (process.env.NODE_ENV === 'development') {
                debug('Server <- Client'.cyan);
                hexout(data);
            }

            const command = self._nextCommand(databuff);
            if (command) {
    ...
    sendMessage(message, subChannel = 0, encoding = 'utf-8') {
        const messageBuffer =
                serverCommands.MESSAGE.pack({ subChannel, message }, encoding);
