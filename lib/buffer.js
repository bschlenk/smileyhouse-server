...
function getSize(val) {
    switch (val.type) {
        ...
        case 'int16': return 2;
        case 'int32': return 4;
        case 'varString': return 4 + Buffer.byteLength(val.value);
        default: return 0;
    }
}

function calcSize(fields) {
    let size = 0;
...
function writeBuffer(buffer, val, offset) {
    switch (val.type) {
        case 'int8':
        ...
        case 'varString':
            offset = buffer.writeInt32LE(Buffer.byteLength(val.value), offset);
            return buffer.write(val.value, offset);
        default:
            return offset;
    }
}
